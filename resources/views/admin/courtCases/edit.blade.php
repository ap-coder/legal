...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.courtCase.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.court-cases.update", [$courtCase->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('client') ? 'is-invalid' : '' }}" name="client_id" id="client_id">
                    @foreach($clients as $id => $client)
                        <option value="{{ $id }}" {{ ($courtCase->client ? $courtCase->client->id : old('client_id')) == $id ? 'selected' : '' }}>{{ $client }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('case_name') ? 'is-invalid' : '' }}" type="text" name="case_name" id="case_name" value="{{ old('case_name', $courtCase->case_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('case_number') ? 'is-invalid' : '' }}" type="text" name="case_number" id="case_number" value="{{ old('case_number', $courtCase->case_number) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('awarded_value') ? 'is-invalid' : '' }}" type="number" name="awarded_value" id="awarded_value" value="{{ old('awarded_value', $courtCase->awarded_value) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('proposed_value') ? 'is-invalid' : '' }}" type="number" name="proposed_value" id="proposed_value" value="{{ old('proposed_value', $courtCase->proposed_value) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('closed_date') ? 'is-invalid' : '' }}" type="text" name="closed_date" id="closed_date" value="{{ old('closed_date', $courtCase->closed_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('completed_date') ? 'is-invalid' : '' }}" type="text" name="completed_date" id="completed_date" value="{{ old('completed_date', $courtCase->completed_date) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('attorneys') ? 'is-invalid' : '' }}" name="attorneys[]" id="attorneys" multiple>
                    @foreach($attorneys as $id => $attorneys)
                        <option value="{{ $id }}" {{ (in_array($id, old('attorneys', [])) || $courtCase->attorneys->contains($id)) ? 'selected' : '' }}>{{ $attorneys }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employees') ? 'is-invalid' : '' }}" name="employees[]" id="employees" multiple>
                    @foreach($employees as $id => $employees)
                        <option value="{{ $id }}" {{ (in_array($id, old('employees', [])) || $courtCase->employees->contains($id)) ? 'selected' : '' }}>{{ $employees }}</option>
                    @endforeach
                </select>
