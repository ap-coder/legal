...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.courtDate.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.court-dates.update", [$courtDate->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('client') ? 'is-invalid' : '' }}" name="client_id" id="client_id">
                    @foreach($clients as $id => $client)
                        <option value="{{ $id }}" {{ ($courtDate->client ? $courtDate->client->id : old('client_id')) == $id ? 'selected' : '' }}>{{ $client }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('court_cases') ? 'is-invalid' : '' }}" name="court_cases[]" id="court_cases" multiple>
                    @foreach($court_cases as $id => $court_case)
                        <option value="{{ $id }}" {{ (in_array($id, old('court_cases', [])) || $courtDate->court_cases->contains($id)) ? 'selected' : '' }}>{{ $court_case }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('courthouse') ? 'is-invalid' : '' }}" name="courthouse_id" id="courthouse_id">
                    @foreach($courthouses as $id => $courthouse)
                        <option value="{{ $id }}" {{ ($courtDate->courthouse ? $courtDate->courthouse->id : old('courthouse_id')) == $id ? 'selected' : '' }}>{{ $courthouse }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('docket_number') ? 'is-invalid' : '' }}" type="text" name="docket_number" id="docket_number" value="{{ old('docket_number', $courtDate->docket_number) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('courtdate') ? 'is-invalid' : '' }}" type="text" name="courtdate" id="courtdate" value="{{ old('courtdate', $courtDate->courtdate) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('needed') ? 'is-invalid' : '' }}" name="needed" id="needed">{!! old('needed', $courtDate->needed) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('outcome') ? 'is-invalid' : '' }}" name="outcome" id="outcome">{!! old('outcome', $courtDate->outcome) !!}</textarea>
